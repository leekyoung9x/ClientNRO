...
public class Arrowpaint
{
  ...
  {
    int dirIndexFromAngle = Arrowpaint.findDirIndexFromAngle(Res.angle(this.axTo - this.ax, -(this.ayTo - this.ay)));
    SmallImage.drawSmallImage(g, this.imgId[(int) Arrowpaint.FRAME[dirIndexFromAngle]], this.ax, this.ay, Arrowpaint.TRANSFORM[dirIndexFromAngle], mGraphics.VCENTER | mGraphics.HCENTER);
  }

  public static int findDirIndexFromAngle(int angle)
  {
    for (int index = 0; index < Arrowpaint.ARROWINDEX.Length - 1; ++index)
    {
      if (angle >= Arrowpaint.ARROWINDEX[index] && angle <= Arrowpaint.ARROWINDEX[index + 1])
