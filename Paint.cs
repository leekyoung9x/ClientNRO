...
public class Paint
{
  ...
  public static void loadbg()
  {
    for (int index = 0; index < Paint.goc.Length; ++index)
      Paint.goc[index] = GameCanvas.loadImage("/mainImage/myTexture2dgoc" + (object) (index + 1) + ".png");
  }
  ...
  public void paintLogo(mGraphics g, int x, int y) => g.drawImage(Paint.imgLogo, x, y, 3);
  ...
  public void paintImgMsg(mGraphics g, int x, int y, int index) => g.drawImage(Paint.imgMsg[index], x, y, 0);

  public void paintTitleBoard(mGraphics g, int roomId) => this.paintDefaultBg(g);
  ...
  public void paintInputDlg(mGraphics g, int x, int y, int w, int h, string[] str)
  {
    ...
    {
      mFont.tahoma_8b.drawString(g, str[index], x + w / 2, y1, 2);
      ++index;
      y1 += mFont.tahoma_8b.getHeight();
    }
  }

  public void paintIconMainMenu(
    mGraphics g,
    int x,
    int y,
    bool iss,
  ...
  public int[] getColorMsg() => this.color;
  ...
  public void paintSellectBoard(mGraphics g, int x, int y, int w, int h) => g.drawImage(Paint.imgSelectBoard, x - 7, y, 0);

  public int isRegisterUsingWAP() => 0;

  public string getCard() => "/vmg/card.on";
  ...
  public string getUrlUpdateGame() => "http://wap.teamobi.com?info=checkupdate&game=3&version=" + GameMidlet.VERSION + "&provider=" + (object) GameMidlet.PROVIDER;
  ...
  public void paintFrameBorder(int x, int y, int w, int h, mGraphics g) => this.paintFrame(x, y, w, h, g);
